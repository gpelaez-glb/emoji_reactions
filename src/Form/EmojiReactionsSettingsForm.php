...
namespace Drupal\emoji_reactions\Form;
...
/**
 * Class EmojiReactionsSettingsForm.
 *
 * @package Drupal\emoji_reactions\Form
 */
class EmojiReactionsSettingsForm extends ConfigFormBase {
  ...
  public function getFormId() {
    return 'emoji_reactions_settings_form';
  }

  /**
   * Constructs a emoji_reactionsSettings object.
   *
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->config('emoji_reactions.settings');
    $form['target_entities'] = [
      '#type' => 'select',
      '#title' => $this->t('Target entities:'),
      '#required' => TRUE,
      '#description' => $this->t('Select the entity content to allow users reacting it.'),
    ...
    $form['after_owner_deletion'] = [
      '#type' => 'select',
      '#title' => $this->t('Set reaction to:'),
      '#required' => TRUE,
      '#description' => $this->t('Select action after reacted content owner account deletion.'),
      '#default_value' => $config->get('after_owner_deletion'),
      '#options' => [
        'delete' => $this->t('Delete reaction content.'),
  ...
  public function isTokenSeedSet() {
    return ($this->config('emoji_reactions.settings')->get('token_seed')) ? TRUE : FALSE;
  }
